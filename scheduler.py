...
if __name__ == '__main__':
    ...
    seed_points = [int(x) for x in list(np.linspace(1, 1000000, num=len(inter_arrival_mu_list)))]
    ...
    random_model_satisfaction_elastic = []
    random_model_satisfaction_rigid = [] 
    
    
    best_model_cpu_utilization_KPI = []
    best_model_mem_utilization_KPI = []
    best_model_satisfaction_elastic = []
    best_model_satisfaction_rigid = []

    
    swarm_model_cpu_utilization_KPI = []
    swarm_model_mem_utilization_KPI = []
    swarm_model_satisfaction_elastic = []
    swarm_model_satisfaction_rigid = []
    ...
    for count, inter_arrival_mu in enumerate(inter_arrival_mu_list):
        ...
        random_model_satisfaction_elastic.append(s11)
        random_model_satisfaction_rigid.append(s12)
        ...
        best_model_satisfaction_elastic.append(s21)
        best_model_satisfaction_rigid.append(s22)
        ...
        swarm_model_satisfaction_elastic.append(s31)
        swarm_model_satisfaction_rigid.append(s32)
  
            

        # if inter_arrival_mu in inter_arrival_mu_list[::2]:
        ...
        #     ax12, ax12 = visualization.plot_dynamics(swarm_model, ax11, ax12, q1_color='g', q2_color='#3CB371', cpu_color='g', mem_color='g', alpha=1.0, label='bottom-up')
            #fig1.savefig('dynamic'+str(inter_arrival_mu)+'.pdf')
    ...
    ax22.plot(inter_arrival_mu_list, random_model_satisfaction_rigid, color='b', label='random-rigid' )
    ax22.plot(inter_arrival_mu_list, best_model_satisfaction_rigid, color='r', label='best-rigid')
    ax22.plot(inter_arrival_mu_list, swarm_model_satisfaction_rigid, color='g', label='bottom-up-rigid')
    ax22.plot(inter_arrival_mu_list, random_model_satisfaction_elastic, color='b', linestyle='--', label='random-elastic' )
    ax22.plot(inter_arrival_mu_list, best_model_satisfaction_elastic, color='r', linestyle='--', label='best-elastic')
    ax22.plot(inter_arrival_mu_list, swarm_model_satisfaction_elastic, color='g', linestyle='--', label='bottom-up-elastic')

    ax22.set_xlabel(r'$\lambda$')
    ax22.set_ylabel('satisfaction rate')
    ax22.set_ylim([-0.01,1.01])
    ax22.set_xlim([-0.0,max(inter_arrival_mu_list)+0.05])
    ax22.legend()
    fig2.savefig('KPI5.pdf')
